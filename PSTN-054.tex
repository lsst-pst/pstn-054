...
\caption{Distribution of readout-noise values for the 3000 amplifiers on the CCD plane of the LSST DOE camera measured for a 2.232 seconds readout. The median noise is measured to be well below the requirement of 9~$e^-$/pixel/readout, between 5 and 6 ~$e^-$/pixel/readout (with a soft dependence on readout speed). However, the distribution shows a long tail with 90\% of the amplifiers within 9~$e^-$/pixel/readout. Some amplifiers (expexted $\sim1$) with poor performance that contribute to the tail, however, will not be used.
  %and   maximum value exceeding 100~$e^-$/pixel/readout.}\label{fig:rndist}
\end{figure}
