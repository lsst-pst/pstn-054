...
\setDocAbstract{%
This document presents updated estimates, as of May 2022, of the Rubin system throughput and compares them to throughput requirements from the LSST Science Requirements Document. In addition, it uses these estimates to
...
the telescope cannot even point towards the zenith such that $X>1$ for all osbervations, and the fiducial sky brightness is approximately equivalent to 
...
We derived the estimates of \mf\ setting a readout noise equal to 8.8  e$^-$/pixel/read-out. This corresponds to asusuming readout noise equal to the system requirements readout noise across the entire CCD plane (see \href{https://docushare.lsst.org/docushare/dsweb/Get/LSE-59}{{LSE-59}}). \footnote{
...
%We derived the estimates of \mf\ setting a readout noise equal to the system requirements readout noise across the entire CCD plane (see \href{https://docushare.lsst.org/docushare/dsweb/Get/LSE-59}{{LSE-59}}). These requirements are for a maximum readout noise of 9 e$^-$/pixel/read-out.\footnote{As the total readout budget includes an estimated 0.2  e$^-$/pixel/read-out of dark currrent, the readout noise is actually assumed to be 8.8  e$^-$/pixel/read-out. This, however, generates negligible differences.}
...
\caption{Distribution of airmass, seeing, sky brightness, and \mf\ for the 10-year LSST survey simulated in \baseline, including Wide-Fast-Deep and minisurveys within.}\label{fig:bv2distributions}
\end{figure}
\FloatBarrier
\begin{table}
